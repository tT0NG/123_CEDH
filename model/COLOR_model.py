...
class Dense_decoder(nn.Module):
    def __init__(self, out_channel):
        ...
        self.refine3 = nn.Conv2d(20 + 4, out_channel, kernel_size=3, stride=1, padding=1)
...
class Dense_color(nn.Module):
    def __init__(self):
        super(Dense_color, self).__init__()
        ...
        self.decoder_r = Dense_decoder(out_channel=1)
        self.decoder_g = Dense_decoder(out_channel=1)
        self.decoder_b = Dense_decoder(out_channel=1)
        ...
        self.refine1_cl = nn.Conv2d(9, 20, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        ######################################
        R = self.decoder_r(x, x1, x2, x4)
        G = self.decoder_g(x, x1, x2, x4)
        B = self.decoder_b(x, x1, x2, x4)
        img_cat = torch.cat([R,G,B], 1)
        dehaze = img_cat
